...
- **Options** — form for `src`, `target` (`auto` | `popup`), `appearance`
  (default `storefront` | `app:light` | `app:dark` | `app:automatic` | `storefront`), and `debug`, plus
...
- **Demo Storefront** — a mocked product card with **Buy now** calling
  ...
  The collapsible readout shows `checkout`, `error`, `target`, `appearance`,
  and `debug`.
