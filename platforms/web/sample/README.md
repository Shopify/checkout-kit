...
- **Options** — form for `src`, `color-scheme`, `target` (`auto` | `popup`),
  and `debug`, plus buttons for `open()`, `close()`, and `focus()`.
- **Demo Storefront** — a mocked product card with **Buy now** calling
  ...
  The collapsible readout shows `checkout`, `error`,
  `colorScheme`, `target`, and `debug`.
