...
{
  static observedAttributes = ["src", "auth", "target"] as const;
  ...
  async #handleReadyRequest(message: CheckoutProtocolMessage): Promise<void> {
    ...
    const readyResponse = normalizeReadyResponse(response);
    if (body?.auth && typeof readyResponse.credential !== "string") {
      postProtocolResult(
        message,
        ucpError(
          "not_supported_error",
          "No credential response was provided for ec.ready",
          "unrecoverable",
        ),
      );
      return;
...
function ucpError(
  code: string,
  content: string,
  severity: UcpErrorResponse["messages"][number]["severity"],
): UcpErrorResponse {
  return {
    ucp: {
      version: EMBED_PROTOCOL_VERSION,
