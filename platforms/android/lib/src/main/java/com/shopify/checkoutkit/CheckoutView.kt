...
public class CheckoutView @MainThread internal constructor(
    context: Context,
    checkoutUrl: String,
    webMessageTransport: WebMessageTransport,
    private val presentation: CheckoutViewPresentation,
) : FrameLayout(context) {
    ...
    init {
        ...
        try {
            val webView = CheckoutWebView.takePreloadedOrCreate(checkoutUrl, context, webMessageTransport)
            checkoutWebView = webView
            webView.markPresented()
            webView.setListener(webViewListener())
            webView.setClient(presentation.protocolClient)
            addWebViewToContainer(webView)
        } catch (error: UnsupportedWebViewException) {
            initializationError = error.checkoutError
            if (presentation.reportInitializationFailure) {
                presentation.onFailure(error.checkoutError)
            }
        }
    }
    ...
    internal fun configureForBottomSheet(sheet: CheckoutSheetOptions) {
        val colorScheme = ShopifyCheckoutKit.configuration.appearance.colorScheme
    ...
    private fun notifyCheckoutCanceled() {
        ...
        presentation.onCancelRequest()
    }

    private fun webViewListener(): CheckoutWebViewListener = CheckoutWebViewListener(
        listener = presentation.listener,
        closeCheckoutWithError = presentation.onFailure,
    ...
    public companion object {
        private const val LOG_TAG = "CheckoutView"
        ...
        public fun create(
            ...
            checkoutUrl = checkoutUrl,
            webMessageTransport = WebMessageListenerTransport,
            presentation = buildCheckoutViewPresentation(configure),
        )
...
internal data class CheckoutViewPresentation(
...
private fun buildCheckoutViewPresentation(
    configure: CheckoutPresentation.() -> Unit,
): CheckoutViewPresentation {
    val presentation = CheckoutPresentation().apply(configure)
    val listener = presentation.buildListener()
    return CheckoutViewPresentation(
        listener = listener,
