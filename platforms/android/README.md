...
- WebMessageListener support in the WebView installed on the buyer's device. This is available in Android System WebView
  ...
  `web_view_not_supported` and returns `null`; an embedded `CheckoutView` invokes `onFail` and remains inert.
...
Use `CheckoutView` when your app owns the presentation container. The view owns the checkout header, close control,
...
Jetpack Compose apps can host `CheckoutView` with `AndroidView`; Checkout Kit does not add a Compose dependency:
...
fun CartScreen(checkoutUrl: String) {
    ...
    ModalBottomSheet(
        ...
        AndroidView(
            factory = { context ->
                CheckoutView.create(context, checkoutUrl) {
...
View-system and Java hosts can construct `CheckoutView(context, checkoutUrl, listener, protocolClient)` directly and
...
`CheckoutView` callbacks and protocol client are fixed when the view is created. Create a new view for a new checkout
