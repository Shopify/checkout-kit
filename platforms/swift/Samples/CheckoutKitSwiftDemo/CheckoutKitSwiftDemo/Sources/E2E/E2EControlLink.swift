...
enum E2EControlLinkError: LocalizedError, Equatable, Hashable {
    ...
    case unexpectedParameters(command: String)
    ...
    var errorDescription: String? {
        ...
        case let .unexpectedParameters(command):
            return "\(command) takes no parameters"
        case .missingProductSelector:
...
enum E2EControlLink: Equatable {
    ...
    static func parse(_ url: String) throws -> E2EControlLink? {
        ...
        let parameters = Parameters(components.queryItems)

        switch components.path.trimmingCharacters(in: CharacterSet(charactersIn: "/")) {
        case "reset":
            guard parameters.isEmpty else {
                throw E2EControlLinkError.unexpectedParameters(command: "reset")
            }
            return .reset
        case "cart":
    ...
    private static func cartCommand(from parameters: Parameters) throws -> CartCommand {
        ...
        guard let productIndex = Int(productIndexParameter), productIndex >= 0 else {
    ...
    private static func quantity(from parameters: Parameters) throws -> Int {
        ...
        guard let quantity = Int(parameter), quantity >= 1 else {
    ...
    private struct Parameters {
        ...
        init(_ queryItems: [URLQueryItem]?) {
            values = (queryItems ?? []).reduce(into: [:]) { result, item in
                result[item.name] = (item.value ?? "").trimmingCharacters(in: .whitespaces)
            }
        }
