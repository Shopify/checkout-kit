...
> production-ready. The current Checkout Kit for Swift alpha is `4.0.0-alpha.7`.
...
The current Checkout Kit for Swift alpha is `4.0.0-alpha.7`. For alpha testing, use that exact version until a stable `4.x` release is available.
...
```ruby
pod "ShopifyCheckoutKit", "4.0.0-alpha.7"

# Optional: Shop Pay and Apple Pay accelerated checkout buttons.
pod "ShopifyCheckoutKit/AcceleratedCheckouts", "4.0.0-alpha.7"
```
...
- `CheckoutKitSwiftDemo` demonstrates a Storefront API cart flow, buyer identity modes, Customer Account API, checkout presentation, checkout lifecycle events, and configurable Shop Pay and Apple Pay accelerated checkout buttons.
