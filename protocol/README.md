...
The fulfillment-option description decoder narrowly accepts the legacy April
string representation, normalizing it into the existing structured `Description`
model with `plain` set and the rich-text representations absent. Re-encoding
canonicalizes that legacy string to `{ "plain": "..." }`; it does not preserve the
original string wire shape. Payment-instrument constraints use the closed
`ConstraintExpression` model, and unknown legacy constraint keys are not promised
