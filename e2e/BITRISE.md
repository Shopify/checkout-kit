...
### The report drives the native check

`ci-ios-report` runs with `should_always_run: workflow` and publishes a `Checkout Kit iOS` sticky PR comment through `e2e/scripts/report_ios_ci_results`. It then exits `0` for a passing evaluation or `1` for a failure, making Bitrise's native `ci/bitrise/ci-ios/pr` check authoritative. It does not create a separate `Checkout Kit iOS` check.

The reporter retains the selection-aware evaluation because Bitrise's workflow status alone cannot tell the two kinds of not-run apart:
...
The reporter also fails when `ci-ios-plan` itself fails or is missing, rather than reporting green off an empty selection. A successful plan selecting no jobs passes. `e2e/test/ios_ci_reporter_test.rb` covers the evaluation, and `e2e/test/report_results_test.rb` verifies publication before failure and the process exit status.
...
Bitrise publishes one native check per PR pipeline: `ci/bitrise/e2e/pr` and `ci/bitrise/ci-ios/pr`. Both names remain the required checks. The `e2e-report` and `ci-ios-report` workflows publish separate sticky PR comments using the short-lived token generated by the Bitrise GitHub App, then exit according to their result evaluations. Failed tests, recorded runner errors, and missing expected results fail `e2e-report`; failed or missing expected iOS stages fail `ci-ios-report`. Neither reporter creates a duplicate Check Run.

Manual branch builds have no pull request to update, so both reporting workflows skip GitHub publication without requiring a token. They still print their summaries and exit according to the results, so failures cannot leave a manual pipeline green.

The Bitrise project has **Project settings > Repository > Extend GitHub App permissions to builds** enabled. Bitrise exposes the build-scoped GitHub App token as `GIT_HTTP_PASSWORD`. GitHub API scripts prefer an explicit `OVERRIDE_GITHUB_TOKEN` for local runs and otherwise use `GIT_HTTP_PASSWORD`; they intentionally ignore the shared `GITHUB_TOKEN` so reporting uses the repository-scoped build identity.

Each reporter maintains its own sticky PR comment with a distinct marker, so the parallel pipelines cannot overwrite each other's summaries. The E2E comment retains "Install with Tophat" links for produced SDK targets and the results table; failing runs add direct BrowserStack evidence links. The iOS CI comment lists passed, failed, and intentionally skipped jobs. Both summaries link to their Bitrise pipeline on success and failure. The native checks also link directly to their pipelines.

The install links and Quick Launch entries are driven by `scripts/tophat/targets.json`; see the Tophat section in `.github/CONTRIBUTING.md`.
